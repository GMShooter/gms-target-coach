...
class SOTAMetricsCalculator:
    ...
    def calculate_group_characteristics(
        ...
        try:
            logger.info(
                f"Calculating group characteristics for {len(shots)} shots"
            )

            if not shots:
                raise InsufficientDataError(
                    "No shots provided for group characteristics calculation"
                )
            ...
            mpi = statistics_calculator.calculate_mean_point_of_impact(
                shots, coords=coords
            )
            extreme_spread = statistics_calculator.calculate_extreme_spread(
                shots, coords=coords
            )
            mean_radius = statistics_calculator.calculate_mean_radius(
                shots, mpi, coords=coords
            )
            std_dev_x, std_dev_y = (
                statistics_calculator.calculate_standard_deviations(
                    shots, coords=coords
                )
            )
            ...
            if len(shots) >= 3:
                characteristics["cep_50"] = (
                    self._calculate_circular_error_probable(shots, 0.5)
                )
                characteristics["cep_90"] = (
                    self._calculate_circular_error_probable(shots, 0.9)
                )
                characteristics["cep_95"] = (
                    self._calculate_circular_error_probable(shots, 0.95)
                )

            # Calculate figure of merit (FOM)
            characteristics["figure_of_merit"] = (
                self._calculate_figure_of_merit(shots)
            )

            logger.info(
                f"Calculated group characteristics: {len(characteristics)} "
                f"metrics"
            )
            ...
            raise AnalysisError(
                f"Failed to calculate group characteristics: {e}"
            )

    def calculate_sequential_metrics(
        self, shots: List[Shot]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"Calculating sequential metrics for {len(shots)} shots"
            )

            if len(shots) < 2:
                raise InsufficientDataError(
                    "At least 2 shots required for sequential analysis"
                )
            ...
            for i in range(1, len(sorted_shots)):
                prev_mpi = (
                    statistics_calculator.calculate_mean_point_of_impact(
                        sorted_shots[:i]
                    )
                )
            ...
            sequential_metrics = {
                ...
                "shot_to_shot_displacement": (
                    np.mean(shot_displacements) if shot_displacements else 0.0
                ),
                "max_shot_displacement": (
                    np.max(shot_displacements) if shot_displacements else 0.0
                ),
                "min_shot_displacement": (
                    np.min(shot_displacements) if shot_displacements else 0.0
                ),
                "shot_displacement_std": (
                    np.std(shot_displacements) if shot_displacements else 0.0
                ),
            ...
            logger.info(
                f"Calculated sequential metrics: {len(sequential_metrics)} "
                f"metrics"
            )
    ...
    def detect_flyers(
        ...
        try:
            logger.info(
                f"Detecting flyers using {method} method with "
                f"threshold {threshold}"
            )

            # Handle ShotGroup input
            if isinstance(shots, ShotGroup):
            ...
            if len(shots_list) < 3:
                logger.warning(
                    "Insufficient shots for reliable flyer detection"
                )
                return shots_list, []

            if method == "std_dev":
                return self._detect_flyers_std_dev(shots_list, threshold)
            elif method == "iqr":
                return self._detect_flyers_iqr(shots_list, threshold)
            elif method == "convex_hull":
                return self._detect_flyers_convex_hull(shots_list, threshold)
            elif method == "distance":
                return self._detect_flyers_distance(shots_list, threshold)
            else:
    ...
    def calculate_scoring_metrics(
        self, shots: List[Shot], target_center: Point, ring_radii: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"Calculating scoring metrics for {len(shots)} shots with {len(ring_radii)} rings"
            )

            if not shots or not ring_radii:
                raise InsufficientDataError(
                    "Shots and ring radii required for scoring calculation"
                )
    
            if target_center is None:
                raise AnalysisError(
                    "Target center required for scoring calculation"
                )
            ...
            logger.info(
                ...
                f"total_score={scoring_metrics['total_score']}"
            )
            return scoring_metrics

        except Exception as e:
            logger.error(f"Failed to calculate scoring metrics: {e}")
            # Re-raise the original exception to preserve type
            raise

    def calculate_comprehensive_sota_metrics(
        self, shot_group: ShotGroup
    ) -> SOTAMetrics:
        """
        Calculate comprehensive SOTA metrics for a shot group.

        Args:
            shot_group: ShotGroup object containing shots and target
            information
        ...
        try:
            logger.info(
                "Calculating comprehensive SOTA metrics for shot group "
                f"with {len(shot_group.shots)} shots"
            )

            if not shot_group.shots:
                raise InsufficientDataError(
                    "No shots provided for SOTA metrics calculation"
                )
            ...
            group_chars = self.calculate_group_characteristics(
                shot_group.shots
            )
            ...
            if len(shot_group.shots) >= 3:
                sota_metrics.convex_hull_area = (
                    statistics_calculator.calculate_convex_hull_area(
                        shot_group.shots
                    )
                )
            ...
            if len(shot_group.shots) >= 2:
                sequential = self.calculate_sequential_metrics(
                    shot_group.shots
                )
                sota_metrics.first_shot_displacement = sequential[
                    "first_shot_displacement"
                ]
                sota_metrics.shot_to_shot_displacement = sequential[
                    "shot_to_shot_displacement"
                ]
            ...
            logger.error(
                f"Failed to calculate comprehensive SOTA metrics: {e}"
            )
            raise AnalysisError(
                f"Failed to calculate comprehensive SOTA metrics: {e}"
            )

    def _calculate_circular_error_probable(
        self, shots: List[Shot], probability: float
    ) -> float:
        """Calculate Circular Error Probable (CEP) for given probability."""
        try:
            coords = np.array([[shot.x, shot.y] for shot in shots])
            # Use median center instead of mean to reduce influence of outliers
            center = np.median(coords, axis=0)
            distances = [np.linalg.norm(coord - center) for coord in coords]

            # Calculate radius that contains given percentage of shots
    ...
    def _calculate_figure_of_merit(self, shots: List[Shot]) -> float:
        ...
        try:
            ...
            fom = (
                (extreme_spread / mean_radius) * 100
                if mean_radius > 0
                else 0.0
            )
            return fom
    ...
    def _detect_flyers_std_dev(
        self, shots: List[Shot], threshold: float
    ) -> Tuple[List[Shot], List[Shot]]:
        ...
        try:
            ...
            center = np.median(coords, axis=0)
            distances = [np.linalg.norm(coord - center) for coord in coords]
            # Use median distance instead of mean to reduce influence of
            # outliers
            ...
            robust_std = (
                mad * 1.4826
            )  # Conversion factor for normal distribution

            good_shots = []
            flyer_shots = []

            for i, shot in enumerate(shots):
                if distances[i] <= median_dist + threshold * robust_std:
    ...
    def _detect_flyers_convex_hull(
        self, shots: List[Shot], threshold: float
    ) -> Tuple[List[Shot], List[Shot]]:
        ...
        try:
            ...
            # Check if points are nearly collinear before creating convex hull
            # Calculate the area of the bounding box
            min_coords = np.min(coords, axis=0)
            max_coords = np.max(coords, axis=0)
            bbox_area = (max_coords[0] - min_coords[0]) * (
                max_coords[1] - min_coords[1]
            )
            # If bounding box area is very small relative to spread,
            # points are nearly collinear
            if bbox_area < 1e-6:
                logger.warning(
                    "Points are nearly collinear, falling back to distance "
                    "method"
                )
            ...
            for i, shot in enumerate(shots):
                ...
                dist_to_hull = self._distance_to_convex_hull(
                    coords[i], hull, coords
                )
            ...
            logger.info(
                f"Convex hull method: {len(flyer_shots)} flyers detected"
            )
            return good_shots, flyer_shots
        except Exception as e:
            logger.error(f"Failed to detect flyers with convex hull: {e}")
            # Fall back to distance method if convex hull fails
            logger.info("Falling back to distance method for flyer detection")
            return self._detect_flyers_distance(shots, threshold)

    def _detect_flyers_distance(
        self, shots: List[Shot], threshold: float
    ) -> Tuple[List[Shot], List[Shot]]:
        """Detect flyers using distance from median method."""
        try:
            coords = np.array([[shot.x, shot.y] for shot in shots])
            # Use median center instead of mean to reduce influence of outliers
            center = np.median(coords, axis=0)
            ...
            for shot in shots:
                ...
                if dist <= median_distance + threshold * robust_std:
                    good_shots.append(shot)
                else:
                    flyer_shots.append(shot)
    ...
    def _distance_to_convex_hull(
        self, point: np.ndarray, hull: ConvexHull, coords: np.ndarray
    ) -> float:
    ...
    def _calculate_shot_score(
        self, distance: float, ring_radii: List[float]
    ) -> int:
        """Calculate score for a single shot based on distance and ring
        radii."""
        try:
