"""Shot analysis module for GMShoot."""
...
from src.utils.exceptions import AnalysisError  # noqa: E402
from src.analysis_engine.models import Shot, ShotGroup  # noqa: E402
...
class ShotAnalyzer:
    ...
    def __init__(self, duplicate_threshold: float = 15.0):
        ...
        Args:
            duplicate_threshold: Distance threshold for considering shots as
            duplicates
        """
        self.duplicate_threshold = duplicate_threshold
        logger.debug(
            f"ShotAnalyzer initialized with duplicate threshold: "
            f"{duplicate_threshold}"
        )

    def find_new_shots(
        self, previous_shots: List[Shot], current_detections: List[Shot]
    ) -> List[Shot]:
        """
        ...
        try:
            logger.info(
                f"Finding new shots: {len(previous_shots)} previous, "
                f"{len(current_detections)} current"
            )
            ...
            for detection in current_detections:
                ...
                for prev_shot in previous_shots:
                    ...
                    if dist <= self.duplicate_threshold:
                        logger.debug(
                            f"Detection at ({detection.x}, {detection.y}) is "
                            f"duplicate of previous shot at ({prev_shot.x}, "
                            f"{prev_shot.y}) with distance {dist}"
                        )
                        is_duplicate = True
                        break
                ...
                if not is_duplicate:
                    ...
                    logger.debug(
                        f"New shot detected at ({detection.x}, {detection.y}) "
    ...
    def merge_shot_groups(self, groups: List[ShotGroup]) -> ShotGroup:
        ...
        try:
            ...
            logger.info(
                f"Merged group contains {len(merged_group.shots)} shots"
            )
            return merged_group
    ...
    def filter_shots_by_confidence(
        self, shots: List[Shot], min_confidence: float = 0.5
    ) -> List[Shot]:
        """
        ...
        try:
            logger.info(
                f"Filtering {len(shots)} shots by confidence >= "
                f"{min_confidence}"
            )

            filtered_shots = [
                shot for shot in shots if shot.confidence >= min_confidence
            ]

            logger.info(
                f"Filtered to {len(filtered_shots)} shots above confidence "
                f"threshold"
            )
    ...
    def remove_outliers(
        ...
        """
        Remove outlier shots from group.

        Args:
            shots: List of shots to analyze
            method: Method for outlier detection ("std_dev", "iqr",
            "convex_hull")
            threshold: Threshold for outlier detection

        Returns:
            Tuple of (good_shots, outliers)
        ...
        try:
            logger.info(
                f"Removing outliers using {method} method with threshold "
                f"{threshold}"
            )
    ...
    def _remove_outliers_std_dev(
        ...
        try:
            ...
            for i, shot in enumerate(shots):
                ...
                if dist <= threshold * np.max(std_dev):
                    filtered_shots.append(shot)
                else:
                    outliers.append(shot)
                    logger.debug(
                        f"Removed outlier shot at ({shot.x}, {shot.y}) "
                        f"with distance {dist} from mean"
                    )

            logger.info(
                f"Removed {len(outliers)} outliers using std dev method"
            )
            return filtered_shots, outliers

        except Exception as e:
            logger.error(f"Failed to remove outliers using std dev: {e}")
            raise AnalysisError(
                f"Failed to remove outliers using std dev: {e}"
            )
    ...
    def _remove_outliers_iqr(
        ...
        try:
            ...
            for shot in shots:
                ...
                x_in_range = (
                    q1_x - threshold * iqr_x
                    <= shot.x
                    <= q3_x + threshold * iqr_x
                )
                y_in_range = (
                    q1_y - threshold * iqr_y
                    <= shot.y
                    <= q3_y + threshold * iqr_y
                )
            ...
            logger.info(
                f"Removed {len(outliers)} outliers using IQR method"
            )
            return filtered_shots, outliers
    ...
    def _remove_outliers_convex_hull(
        self, shots: List[Shot], threshold: float
    ) -> tuple[List[Shot], List[Shot]]:
        ...
        try:
            if len(shots) < 4:
                logger.warning(
                    "Insufficient shots for convex hull outlier removal"
                )
                return shots, []
            ...
            for i, shot in enumerate(shots):
                ...
                if dist_to_hull <= threshold:
                    ...
                    logger.debug(
                        f"Removed outlier shot at ({shot.x}, {shot.y}) "
                        f"with distance {dist_to_hull} from convex hull"
                    )

            logger.info(
                f"Removed {len(outliers)} outliers using convex hull method"
            )
            return filtered_shots, outliers
            ...
            raise AnalysisError(
                f"Failed to remove outliers using convex hull: {e}"
            )
