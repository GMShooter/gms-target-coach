...
from src.analysis_engine.models import Shot, Point, ShotGroup, AnalysisSession
...
class StateManager:
    ...
    def set_shots(self, shots: List[Shot]) -> None:
        ...
        try:
            ...
            self._add_to_analysis_history('shots_updated', {'shot_count': len(shots)})
        except Exception as e:
    ...
    def set_filtered_shots(self, filtered_shots: List[Shot]) -> None:
        ...
        try:
            ...
            logger.info(f"Updated filtered shots in session state: {len(filtered_shots)} shots")
            self._add_to_analysis_history('filtering', {'filtered_count': len(filtered_shots)})
        except Exception as e:
            logger.error(f"Failed to set filtered shots in session state: {e}")
            raise ValidationError(f"Failed to set filtered shots in session state: {e}")
    ...
    def set_current_image(self, image: Any) -> None:
        ...
        try:
            ...
            raise ValidationError(f"Failed to set current image in session state: {e}")
    
    def get_mpi(self) -> Optional[Point]:
        """Get MPI from session state."""
        mpi_data = st.session_state.get('mpi', None)
        if mpi_data:
            return Point(x=mpi_data['x'], y=mpi_data['y']) if isinstance(mpi_data, dict) else mpi_data
        return None
    ...
    def set_mpi(self, mpi: Optional[Point]) -> None:
        ...
        try:
            ...
            self._add_to_analysis_history('mpi_calculated', {'mpi': {'x': mpi.x, 'y': mpi.y} if mpi else None})
        except Exception as e:
    ...
    def set_analysis_mode(self, mode: str) -> None:
        ...
        try:
            ...
            raise ValidationError(f"Failed to set analysis mode in session state: {e}")
    ...
    def set_duplicate_threshold(self, threshold: float) -> None:
        ...
        try:
            if threshold < 5.0 or threshold > 50.0:
                raise ValidationError(f"Duplicate threshold must be between 5.0 and 50.0: {threshold}")
            
            st.session_state.duplicate_threshold = threshold
            logger.info(f"Updated duplicate threshold in session state: {threshold}")
            self._add_to_analysis_history('threshold_changed', {'type': 'duplicate', 'value': threshold})
        except Exception as e:
            logger.error(f"Failed to set duplicate threshold in session state: {e}")
            raise ValidationError(f"Failed to set duplicate threshold in session state: {e}")
    ...
    def set_min_confidence(self, confidence: float) -> None:
        ...
        try:
            if confidence < 0.0 or confidence > 1.0:
                raise ValidationError(f"Minimum confidence must be between 0.0 and 1.0: {confidence}")
            
            st.session_state.min_confidence = confidence
            logger.info(f"Updated minimum confidence in session state: {confidence}")
            self._add_to_analysis_history('threshold_changed', {'type': 'confidence', 'value': confidence})
        except Exception as e:
            logger.error(f"Failed to set minimum confidence in session state: {e}")
            raise ValidationError(f"Failed to set minimum confidence in session state: {e}")
    ...
    def set_outlier_method(self, method: str) -> None:
        ...
        try:
            valid_methods = ['None', 'Standard Deviation', 'IQR', 'Convex Hull']
            ...
            self._add_to_analysis_history('outlier_method_changed', {'method': method})
        except Exception as e:
            logger.error(f"Failed to set outlier method in session state: {e}")
            raise ValidationError(f"Failed to set outlier method in session state: {e}")
    ...
    def set_outlier_threshold(self, threshold: float) -> None:
        ...
        try:
            if threshold < 0.5 or threshold > 5.0:
                raise ValidationError(f"Outlier threshold must be between 0.5 and 5.0: {threshold}")
            
            st.session_state.outlier_threshold = threshold
            logger.info(f"Updated outlier threshold in session state: {threshold}")
            self._add_to_analysis_history('threshold_changed', {'type': 'outlier', 'value': threshold})
        except Exception as e:
            logger.error(f"Failed to set outlier threshold in session state: {e}")
            raise ValidationError(f"Failed to set outlier threshold in session state: {e}")
    ...
    def _add_to_analysis_history(self, action: str, data: Dict[str, Any]) -> None:
    ...
    def save_session(self, filename: Optional[str] = None) -> str:
        ...
        try:
            ...
            state_keys = [
                'shots', 'filtered_shots', 'analysis_mode', 
                'duplicate_threshold', 'min_confidence', 
            ...
            if 'filtered_shots' in session_data['state']:
                session_data['state']['filtered_shots'] = [
                    shot.to_dict() for shot in session_data['state']['filtered_shots']
                ]
    ...
    def validate_state(self) -> bool:
        ...
        try:
            ...
            if not isinstance(st.session_state.get('duplicate_threshold', 15.0), (int, float)):
                logger.error("Duplicate threshold is not a number")
                return False
            
            # Validate value ranges
            if st.session_state.get('min_confidence', 0.5) < 0 or st.session_state.get('min_confidence', 0.5) > 1:
