...
from src.utils.exceptions import GMShootError
...
def main():
    ...
    try:
        ...
        analysis_mode = controls_component.render_analysis_mode_selector()
        
        # Configuration section
        config = controls_component.render_configuration_section()
        ...
        if button_states.get('load_clicked', False):
            logger.info(f"Load Image button clicked with mode: {state_manager.get_analysis_mode()}")
            handle_image_load(state_manager.get_analysis_mode())
        
        if button_states.get('analyze_clicked', False):
            logger.info(f"Analyze button clicked")
            handle_analysis(
        ...
        if state_manager.get_current_image() is not None and state_manager.get_shots():
...
def handle_image_load(analysis_mode: str) -> None:
    ...
    try:
        ...
        if analysis_mode == "Local Files":
            ...
            if uploaded_file is not None:
                ...
                if os.path.exists(test_image_path):
                    ...
                    st.session_state.shots = shots
                    st.success(f"Using test image - Detected {len(shots)} shots")
                else:
            ...
            logger.info(f"Live frame result: {image}")
        ...
        st.error(f"An unexpected error occurred while loading the image. Please check the logs.")
...
def handle_analysis(
    ...
    try:
        ...
        filtered_shots = shot_analyzer.filter_shots_by_confidence(shots, min_confidence)
        
        # Remove outliers
        if outlier_method != "None":
            filtered_shots = shot_analyzer.remove_outliers(
        ...
        if len(filtered_shots) > 0:
            ...
            st.success(f"Analysis complete: {len(filtered_shots)} shots after filtering")
        else:
        ...
        st.error(f"An unexpected error occurred during analysis. Please check the logs.")
