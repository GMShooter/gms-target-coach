...
import { Upload, Video, FileVideo, Zap, Brain, Clock } from 'lucide-react';
...
export const VideoUpload: React.FC<VideoUploadProps> = ({ onVideoUpload, onAnalysisComplete }) => {
  ...
  const handleDrillStart = () => {
    setIsRecording(true);
    console.log('Drill mode started with intelligent shot detection');
  };

  const handleDrillStop = () => {
    setIsRecording(false);
    console.log('Drill mode stopped - processing with visual detection');
  };
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-lg p-12 text-center">
          ...
          <h3 className="text-2xl font-bold mb-4 flex items-center justify-center gap-2">
            ...
            Intelligent Shot Analysis
          </h3>
          <div className="space-y-2 text-slate-400">
            <p className="text-lg font-semibold text-blue-400">{analysisProgress}</p>
            <div className="grid grid-cols-1 gap-2 text-sm">
              <p>🎯 Advanced visual shot detection with frame differencing</p>
              <p>🔍 Smart key frame extraction (only critical moments)</p>
              <p>🤖 AI model orchestration (Gemini ↔ Gemma fallback)</p>
              <p>⚡ Optimized API usage - maximum efficiency</p>
              <p>📊 Professional ballistics analysis</p>
            </div>
          </div>
          <div className="mt-6 text-sm text-slate-500 bg-slate-700/30 rounded-lg p-3">
            <p className="font-semibold text-slate-300">Revolutionary Efficiency</p>
            <p>Instead of analyzing hundreds of frames, we detect and send only the exact frames where shots occur!</p>
          </div>
        </div>
      </div>
    );
        ...
        <>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div>
                ...
                <p className="text-slate-400 mb-4">
                  Revolutionary shot detection with visual frame differencing
                </p>
          ...
          <div className="bg-slate-800/30 border border-slate-700 rounded-lg p-6">
            <h4 className="font-semibold mb-3 flex items-center gap-2">
              ...
              Revolutionary Analysis System
            </h4>
            <ul className="space-y-2 text-sm text-slate-400">
              <li>• <strong className="text-blue-400">Visual Shot Detection:</strong> Frame differencing identifies exact impact moments</li>
              <li>• <strong className="text-yellow-400">Smart API Orchestration:</strong> Gemini 2.5 Flash ↔ Gemma 3 fallback system</li>
              <li>• <strong className="text-green-400">Maximum Efficiency:</strong> Only sends critical frames - 99% API cost reduction</li>
              <li>• <strong className="text-purple-400">Instant Processing:</strong> 10 shots = 10 frames instead of 1000+ frames</li>
              <li>• Target must be clearly visible with good contrast and lighting</li>
              <li>• Camera positioned for clear bullet hole visibility</li>
              <li>• Minimum 720p video quality, optimal lighting conditions</li>
              <li>• <strong className="text-red-400">Expert Coaching:</strong> Professional marksmanship analysis</li>
            </ul>
          </div>
        </>
      )}
