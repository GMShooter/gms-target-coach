...
class PerformanceMonitor:
    ...
    def track_processing_time(self, operation: str, duration: float) -> None:
        ...
        with self.lock:
            ...
            logger.debug(
                f"Processing time tracked: {operation} - {duration:.3f}s"
            )
    ...
    def track_response_time(self, endpoint: str, response_time: float) -> None:
        ...
        with self.lock:
            ...
            logger.debug(
                f"Response time tracked: {endpoint} - {response_time:.3f}s"
            )
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if self.metrics["memory_usage"]:
                avg_memory = sum(
                    p["memory_mb"] for p in self.metrics["memory_usage"]
                ) / len(self.metrics["memory_usage"])

            avg_cpu = 0
            if self.metrics["cpu_usage"]:
                avg_cpu = sum(
                    p["cpu_percent"] for p in self.metrics["cpu_usage"]
                ) / len(self.metrics["cpu_usage"])

            avg_response_time = 0
            if self.metrics["response_times"]:
                avg_response_time = sum(
                    p["response_time"] for p in self.metrics["response_times"]
...
def monitor_performance(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            global_performance_monitor.track_processing_time(
...
def start_background_monitoring(interval: int = 30) -> None:
    ...
    logger.info(
        f"Background performance monitoring started with {interval}s interval"
    )
