...
class DataSource:
    ...
    def get_local_frame(
        self, frame_index: int, folder: str = "test_frames"
        ...
        try:
            ...
            if not validate_image_format(image):
                raise ImageProcessingError(
                    f"Invalid image format: {frame_path}"
                )
    ...
    def get_available_local_frames(
        self, folder: str = "test_frames"
    ) -> List[int]:
        """
        ...
        try:
            ...
            for file_path in frame_dir.glob("frame_*.jpg"):
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not parse frame number from: {file_path}"
                    )
                    continue
