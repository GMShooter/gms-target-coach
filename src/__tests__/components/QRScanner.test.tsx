...
// Mock the qr-scanner library with proper ES module handling
const mockQrScanner = {
...
// Add simulateScan method to the constructor for tests
...
// Mock the qr-scanner module with proper default export
...
// Also add the QrScanner to the global scope for type checking
...
describe('QRScanner Component', () => {
  ...
  it('renders QR scanner interface', async () => {
    ...
    expect(screen.getByText(/Position the QR code within the frame to connect to your device/)).toBeInTheDocument();
  ...
  it('shows camera not available when camera access fails', async () => {
    ...
    // Try to start scanning - this should trigger the camera error
    const startButton = screen.getByText('Start Scanning');
    
    // Mock the QrScanner constructor to create a failing instance
    ...
    // First, check if the error message appears in the normal interface
    ...
    await waitFor(() => {
      expect(mockOnScan).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.stringContaining('gms-device'),
  ...
  it('handles successful QR code scan', async () => {
    ...
    // Click start scanning - this will create the scanner instance
    const startButton = screen.getByText('Start Scanning');
    fireEvent.click(startButton);
  ...
  it('handles invalid QR code', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Invalid QR code data. Please scan a valid GMShooter device QR code.')).toBeInTheDocument();
  ...
  it('cleans up camera on unmount', async () => {
    ...
    // Start scanning to create the scanner instance
    const startButton = screen.getByText('Start Scanning');
    fireEvent.click(startButton);
    ...
    // Verify cleanup was called on the instance
  ...
  it('handles QR code with missing required fields', async () => {
    ...
    // Click start scanning button first
    fireEvent.click(screen.getByText('Start Scanning'));
    
    // Wait for scanner to be active - the scanner needs to be created first
    await waitFor(() => {
      expect(screen.getByText('Stop Scanning')).toBeInTheDocument();
    }, { timeout: 5000 });
    ...
    await waitFor(() => {
      expect(screen.getByText('Invalid QR code format. Please scan a valid GMShooter device QR code.')).toBeInTheDocument();
      expect(mockOnScan).not.toHaveBeenCalled();
    });
  });
  ...
  it('adds new device to paired devices after successful scan', async () => {
    ...
    // Wait for scanner to initialize - the scanner needs to be created first
    await waitFor(() => {
      expect(screen.getByText('Stop Scanning')).toBeInTheDocument();
    }, { timeout: 5000 });
