...
def resize_image(
    image: Image.Image, max_size: Tuple[int, int] = (1024, 1024)
) -> Image.Image:
    """
...
def draw_shots_on_image(
    ...
    try:
        ...
        if mpi:
            ...
            draw.line(
                [mx - mpi_size, my, mx + mpi_size, my], fill=mpi_color, width=3
            )
            draw.line(
                [mx, my - mpi_size, mx, my + mpi_size], fill=mpi_color, width=3
            )
...
def validate_image_format(image: Image.Image) -> bool:
    ...
    try:
        ...
        logger.debug(
            f"Image format validation passed: {image.mode}, {image.size}"
        )
        return True
    except Exception as e:
