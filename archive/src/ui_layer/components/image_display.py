...
class ImageDisplay:
    ...
    def display_single_image(
        ...
        try:
            ...
            annotated_image = draw_shots_on_image(
                image, shots, mpi, shot_color="blue", mpi_color="red"
            )
                ...
                shot_data.append(
                    {
                        ...
                        "Frame": (
                            shot.frame_index if shot.frame_index else "N/A"
                        ),
                    }
                )
    ...
    def display_shot_statistics(self, shots: List[Shot]) -> None:
        ...
        try:
            ...
            with col1:
                st.write(
                    f"X Range: {min(x_coords):.1f} to {max(x_coords):.1f}"
                )

            with col2:
                st.write(
                    f"Y Range: {min(y_coords):.1f} to {max(y_coords):.1f}"
                )

        except Exception as e:
            logger.error(f"Failed to display shot statistics: {e}")
            st.error(f"Error displaying shot statistics: {e}")
    def render(self, state_manager=None) -> None:
        """
        Render image display component based on current state.

        This method checks session state and displays appropriate
        image view based on what data is available.
        """
        try:
            # Use provided state_manager or import global one
            ...
            # If no image is loaded, show info message
            if not current_image:
                st.info("No image loaded. Please load an image to begin analysis.")
                return
            ...
            # Display image with shots and MPI
            self.display_single_image(image, shots or [], mpi)
            
        except Exception as e:
            logger.error(f"Failed to render image display component: {e}")
            st.error(f"Error rendering image display: {e}")
