...
class MetricsPanelComponent:
    ...
    def render_basic_metrics(self, shots: List) -> None:
        ...
        try:
            ...
            with col4:
                st.metric(
                    "Combined Std Dev",
                    f"{np.sqrt(std_dev_x**2 + std_dev_y**2):.1f} px",
                )
    ...
    def render_advanced_metrics(self, shots: List) -> None:
        ...
        try:
            ...
            if "figure_of_merit" in group_chars:
                st.metric(
                    "Figure of Merit", f"{group_chars['figure_of_merit']:.1f}"
                )
            ...
            if len(shots) >= 2:
                ...
                with col1:
                    st.metric(
                        "First Shot Displacement",
                        f"{sequential_metrics['first_shot_displacement']:.1f} px",
                    )

                with col2:
                    st.metric(
                        "Shot-to-Shot Displacement",
                        f"{sequential_metrics['shot_to_shot_displacement']:.1f} px",
                    )
                ...
                if "trend_stability" in sequential_metrics:
                    st.metric(
                        "Trend Stability",
                        f"{sequential_metrics['trend_stability']:.3f}",
                    )
        ...
        try:
            ...
            if len(shots) >= 3:
                ...
                for t in theta:
                    x = (
                        mean[0]
                        + np.sqrt(chi2 * eigenvalues[0])
                        * np.cos(t)
                        * eigenvectors[0, 0]
                        - np.sqrt(chi2 * eigenvalues[1])
                        * np.sin(t)
                        * eigenvectors[0, 1]
                    )
                    y = (
    ...
    def render_confidence_intervals(self, shots: List) -> None:
        ...
        try:
            logger.info(
                f"Rendering confidence intervals for {len(shots)} shots"
            )

            if len(shots) < 3:
                st.warning(
                    "At least 3 shots required for confidence intervals"
                )
                return
            ...
            interval_data = {
                ...
                "Range": [
                    f"{intervals['x'][1] - intervals['x'][0]:.2f}",
                    f"{intervals['y'][1] - intervals['y'][0]:.2f}",
                ],
            }
    ...
    def render_flyer_analysis(self, shots: List) -> None:
        ...
        try:
            ...
            # Detect flyers
            good_shots, flyer_shots = sota_metrics_calculator.detect_flyers(
                shots,
                outlier_method.lower().replace(" ", "_"),
                outlier_threshold,
            )
            ...
            with col2:
                st.metric(
                    "Flyer Rate", f"{len(flyer_shots)/len(shots)*100:.1f}%"
                )
    ...
    def render_scoring_metrics(self, shots: List) -> None:
        ...
        try:
            ...
            scoring = sota_metrics_calculator.calculate_scoring_metrics(
                shots, target_center, ring_radii
            )
        ...
        try:
            ...
            if format == "JSON":
                data = {
                    ...
                    "exported_at": str(
                        st.session_state.get("export_timestamp", "")
                    ),
                }
                st.download_button(
                    label="Download JSON",
                ...
                # Write header
                writer.writerow(
                    ["Shot #", "X", "Y", "Confidence", "Frame", "Timestamp"]
                )
                    ...
                    [
                        {
                            ...
                            "Frame": (
                                shot.frame_index if shot.frame_index else None
                            ),
                            "Timestamp": (
                                shot.timestamp.isoformat()
                                if shot.timestamp
                ...
                writer: pd.ExcelWriter = pd.ExcelWriter(excel_output, engine="openpyxl")
            ...
            st.session_state.export_timestamp = str(
                st.session_state.get("export_timestamp", "")
            )
    ...
    def render(self, shots: Optional[List] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if shots is None:
                shots = (
                    state_manager.get_filtered_shots()
                    or state_manager.get_shots()
                )

            if not shots:
                st.info(
                    "No shots to display. Load an image and detect shots first."
                )
                return {}
