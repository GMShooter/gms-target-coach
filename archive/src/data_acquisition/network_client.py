...
class NetworkClient:
    ...
    def get_latest_frame(self) -> Optional[bytes]:
        ...
        try:
            logger.info(
                f"Fetching latest frame from ngrok server. Base URL: "
                f"{self.base_url}"
            )
            ...
            response = requests.get(
                url, headers=self.headers, auth=self.auth, timeout=self.timeout
            )
            ...
            logger.info(
                f"Successfully fetched frame, size: {len(response.content)} "
                f"bytes"
            )
    ...
    def get_frame_next(
        self, since_id: Optional[int] = None, timeout: int = 12
        ...
        try:
            ...
            logger.info(
                f"Successfully fetched frame {frame_id}, "
    ...
    def start_session(
        ...
        try:
            logger.info(
                f"Starting session: preset={preset}, distance={distance}, "
                f"fps={fps}"
            )
    ...
    def stop_session(self) -> dict:
        ...
        try:
            ...
            response = requests.post(
                url, json={}, headers=self.headers, auth=self.auth, timeout=5
            )
    ...
    def _validate_frame_rate(self) -> None:
        ...
        if self.last_frame_time is not None:
            ...
            if actual_interval < expected_interval - tolerance:
                logger.warning(
                    f"Frame arrived too early: {actual_interval:.2f}s "
                    f"(expected ~{expected_interval}s)"
                )
            elif actual_interval > expected_interval + tolerance:
                logger.warning(
                    f"Frame arrived too late: {actual_interval:.2f}s "
                    f"(expected ~{expected_interval}s)"
                )
            else:
                logger.debug(
                    f"Frame rate within tolerance: {actual_interval:.2f}s"
                )
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                url, headers=self.headers, auth=self.auth, timeout=5
            )
